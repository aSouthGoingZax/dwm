...
drawbar(Monitor *m)
{
	...
	/* draw status first, it will be drawn over later by tags */
	if (m == selmon) { /* status is drawn only on selected monitor */
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			...
			if (m->sel->isfloating)
				...
				drw_rect(drw, x + boxs, 0, w, boxw, m->sel->isfixed, 0);
		} else {
...
setup(void)
{
	...
	root = RootWindow(dpy, screen);	xinitvisual();
	drw = drw_create(dpy, screen, root, sw, sh, visual, depth, cmap);
	if (!drw_fontset_create(drw, fonts, LENGTH(fonts)))
		die("no fonts could be loaded.");
	lrpad = drw->fonts->h;
	bh = drw->fonts->h + bhpad;
	updategeom();
	/* init atoms */
